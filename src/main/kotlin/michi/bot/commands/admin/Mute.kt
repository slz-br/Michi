...
object Mute: MichiCommand("mute", "Mutes the mentioned member for a specific period of time.", CommandScope.GUILD_SCOPE) {
    ...
    override val arguments: List<MichiArgument>
        get() = listOf(
            MichiArgument("user", "The user to mute", OptionType.USER, isRequired = true, hasAutoCompletion = false),
    ...
    override fun execute(context: SlashCommandInteractionEvent) {
        ...
        when (time) {
            "1 min" ->  {
                ...
                context.reply("${sender.asMention} muted ${memberToMute.asMention} for 1 minute").queue()
            }
            "5 min" ->  {
                memberToMute.timeoutFor(Duration.ofMinutes(5)).queue()
                context.reply("${sender.asMention} muted ${memberToMute.asMention} for 5 minutes").queue()
            }
            "10 min" -> {
                memberToMute.timeoutFor(Duration.ofMinutes(10)).queue()
                context.reply("${sender.asMention} muted ${memberToMute.asMention} for 10 minutes").queue()
            }
            "1 hour" -> {
                memberToMute.timeoutFor(Duration.ofHours(1)).queue()
                context.reply("${sender.asMention} muted ${memberToMute.asMention} for 1 hour").queue()
            }
            "1 day"  -> {
                memberToMute.timeoutFor(Duration.ofDays(1)).queue()
                context.reply("${sender.asMention} muted ${memberToMute.asMention} for 1 day").queue()
            }
            "1 week" -> {
                memberToMute.timeoutFor(Duration.ofDays(7)).queue()
                context.reply("${sender.asMention} muted ${memberToMute.asMention} for 1 week").queue()
            }
            else -> {
                context.reply("Type a valid option ${Emoji.smolMichiAngry}").setEphemeral(true).queue()
                return
            }
        }
    }
    ...
    override fun canHandle(context: SlashCommandInteractionEvent): Boolean {
        ...
        if (memberToMute == sender) {
            context.reply("Why are you trying to mute yourself? ${Emoji.michiHMPH}").setEphemeral(true).queue()
            return false
        }

        if (memberToMute == bot) {
            context.reply("Stop trying to mute me! ${Emoji.smolMichiAngry}").setEphemeral(true).queue()
            return false
        }
        ...
        if (!sender.permissions.any { permission -> userPermissions.contains(permission) }) {
            context.reply("You don't have permission to use this command, silly you ${Emoji.michiBlep}").setEphemeral(true).queue()
            return false
        }
        ...
        memberToMute?.let {
            ...
            if (memberToMute.roles.any { role -> role.position >= senderTopRole || role.position >= botTopRole }) {
                context.reply("${memberToMute.asMention} has a greater role than you or me").setEphemeral(true).queue()
                return false
            }
        ...
        context.reply("Couldn't find the user in the guild").setEphemeral(true).queue()
        return false
    }
